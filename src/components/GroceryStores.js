...
const PopupContent = (props) => {
    return(<><b>Closest Grocery Store: </b> {props.store.name}, {props.store.distance.toFixed(2)} miles away</>)
}

export default function GroceryStores() {
    const data = useStaticQuery(graphql`
        query GroceryQuery {
            allFoodRetailers2016Csv(filter: {prim_type: {eq: "Supermarkets/Other Grocery (Exc Convenience) Strs"}, recordtype: {eq: "Verified"}}) {
                nodes {
                    longitude
                    latitude
                    name
    ...
    data.tractsBostonBariLayer.features.map((feature) => {
        ...
        const store_list = data.allFoodRetailers2016Csv.nodes.map((store) => {
            ...
            return({"name": store.name, "distance": MeterToMile(nbhd_point.distanceTo(L.latLng(coords)))})
        })
