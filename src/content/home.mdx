...
The purpose of this project was to create a tool where we could essentially "heatmap" on a tract-by-tract basis the availability of different services on a fifteen-minute radius.
This has a broad range of potential users. For example, this analysis and visualization could be used by city planners when determing where increasing density of housing is more
efficient, or determining how to rezone portions of a city that are in underserved areas of the city. Potential renters or home buyers could use this tool to make decisions about
...
have different racial breakdowns? Different median household incomes? With this line of analysis, questions can potentially be answered about equity in the city.
...
-- so, respectively, 5, 20, 45, 75, and 105 minutes for each category.[^7] We then sum `category_proportion * total_population * midpoint` for each category, and divide by the total total_population
...
from the Metropolitan Area Planning Council[^8], found [here](https://data-metroboston.opendata.arcgis.com/datasets/a5d09c31d88b4c96a21fc124115da0e3_0). This data has been broken down into several categories
...
Boston's data portal offers us a data set with the location of each library in the Boston Public Library system.[^11] This map uses this data to show the
...
"every school within a one-mile radius of their home plus, as needed, nearby high-quality schools based on BPS's School Quality Framework (SQF) system of measurement".[^12]
...
in the city.[^13] The dataset lists the type of school in regards to what grades it offers -- "ES" (elementary school), "MS" (middle school), "HS" (high school), "K-8", "6/7-12", "Special", "K-12". For this 
...
in the city.[^14] We have again omitted parts of this data for clarity -- several universities listed by the city are since listed as closed, and there were several double locations of other universities
...
"contain a majority of medical-surgical, pediatric, obstetric, and maternity beds, as defined by the Massachusetts Department of Public Health (DPH)".[^15]
...
We'll turn next to MassGIS data on community health centers, which cover a broad range of other non-acute medical services.[^16] MassGIS uses the definition
...
including medical, social and/or mental health services, to anyone in need regardless of their medical status, ability to pay, culture or ethnicity"[^16]
...
The final point of our analysis is leisure, and in general, quality of life within the neighborhoods of Boston. The Boston Data Portal offers us data of all the public spaces within Boston.[^17]
...
To do that, we'll use the Trees dataset from the Boston Data Portal.[^18] Here, we've parsed the tree data from the city to determine how many trees are in each tract. On the map, green is > 2000 trees, and brown is < 100 trees. 
...
Perhaps not surprisingly, for anyone who has commuted in the city, workplace access was the metric that failed the worst on the 15-minute test. Almost none of Boston's tracts met this criteria --
...
these concerns, with plans to put all city residents within a 10-minute walk of bikeshare, transit, and carshare services, and plans to reduce commute times by 10%.[^19] These incremental efforts will
...
[^7]: Displayr. “How to Calculate an Average Value from Categorical Data,” June 18, 2018. https://www.displayr.com/how-to-calculate-an-average-value-from-categorical-data/.
[^8]: “Food Retailers.” Metropolitan Area Planning Council Open Data, 2016. https://data-metroboston.opendata.arcgis.com/datasets/a5d09c31d88b4c96a21fc124115da0e3_0.
